...
class Ideone
{
	public static final double RI = 0.9;
	
	public static void main (String[] args) throws java.lang.Exception
	{
		double[] preferences = new double[] {3.0, 7.0, 4.0, 5.0};
		
		double[][] weights = new double[4][4];
		
		for (int i = 0; i < weights.length; i++) {
			for (int j = 0; j < weights[0].length; j++) {
				weights[i][j] = preferences[j] > preferences[i] ? 
					1 / (preferences[j] - preferences[i] + 1) : preferences[i] - preferences[j] + 1;
			}
		}
		...
		double[] s = new double[] {
			...
			rowSum(normalizedWeights, 3) / ((double) n)
		};
